...
body { font-family: 'Segoe UI', sans-serif; background: #f5f3ff; min-height: 100vh; margin: 0; }

/* SIDEBAR */
.sidebar {
    width: 240px; min-height: 100vh;
    background: linear-gradient(180deg, #2e1065 0%, #6d28d9 60%, #a78bfa 100%);
...
.page-header h4 { font-size: 1.4rem; font-weight: 700; color: #6d28d9; margin-bottom: 4px; }
.page-header p { color: #6c757d; font-size: 0.85rem; margin: 0; }
.page-header .breadcrumb { font-size: 0.8rem; margin-bottom: 6px; }
.page-header .breadcrumb-item a { color: #8b5cf6; text-decoration: none; font-weight: 500; }
...
.stat-card {
    ...
    box-shadow: 0 2px 16px rgba(109,40,217,0.08); background: #fff;
    transition: transform 0.15s, box-shadow 0.15s;
}
.stat-card:hover { transform: translateY(-3px); box-shadow: 0 8px 24px rgba(109,40,217,0.14); }
...
.icon-primary { background: #f5f3ff; color: #7c3aed; }

.num-total   { color: #6d28d9; } .num-warning { color: #f59e0b; }
.num-success { color: #16a34a; } .num-danger  { color: #dc2626; }
.num-info    { color: #d97706; } .num-primary { color: #7c3aed; }

.lbl-total   { color: #9ca3af; } .lbl-warning { color: #92400e; }
.lbl-success { color: #166534; } .lbl-danger  { color: #991b1b; }
.lbl-info    { color: #d97706; } .lbl-primary { color: #6d28d9; }

/* TABLE CARD */
.table-card { background: #fff; border-radius: 16px; box-shadow: 0 2px 16px rgba(109,40,217,0.08); overflow: hidden; }
.table-card-header {
    padding: 20px 24px; border-bottom: 1px solid #ddd6fe;
    display: flex; align-items: center; justify-content: space-between;
}
.table-card-header h6 { font-size: 1rem; font-weight: 700; color: #6d28d9; margin: 0; }
.badge-count { background: #ddd6fe; color: #6d28d9; font-size: 0.75rem; font-weight: 600; padding: 4px 10px; border-radius: 20px; }

table.data-table { width: 100%; border-collapse: collapse; }
table.data-table thead th {
    background: #f5f3ff; color: #7c3aed; font-size: 0.72rem; font-weight: 700;
    text-transform: uppercase; letter-spacing: 0.6px; padding: 12px 16px;
    border-bottom: 1px solid #ddd6fe; white-space: nowrap;
}
...
table.data-table tbody tr:hover td { background: #f5f3ff; }
...
.badge-primary   { background: #f5f3ff; color: #6d28d9; }
.badge-secondary { background: #f3f4f6; color: #6b7280; }

/* BUTTONS */
.btn-primary-dark {
    display: inline-flex; align-items: center; gap: 8px; background: #6d28d9;
...
.btn-primary-dark:hover { background: #7c3aed; color: #fff; transform: translateY(-1px); box-shadow: 0 4px 14px rgba(109,40,217,0.35); }
...
/* FORM CARD */
.form-card { background: #fff; border-radius: 16px; box-shadow: 0 2px 16px rgba(109,40,217,0.08); overflow: hidden; }
.form-section-header {
    background: #f5f3ff; padding: 14px 24px; border-bottom: 1px solid #ddd6fe;
    display: flex; align-items: center; gap: 10px;
}
.form-section-header .sec-label {
    background: #6d28d9; color: #fff; font-size: 0.72rem; font-weight: 700;
    padding: 3px 9px; border-radius: 6px; letter-spacing: 0.5px;
}
.form-section-header .sec-title { font-size: 0.9rem; font-weight: 700; color: #6d28d9; }
...
.form-control-custom {
    border: 1px solid #c4b5fd; border-radius: 10px; padding: 10px 14px;
    font-size: 0.875rem; color: #374151; width: 100%;
    transition: border-color 0.15s, box-shadow 0.15s; outline: none;
    background: #fff;
}
.form-control-custom:focus { border-color: #8b5cf6; box-shadow: 0 0 0 3px rgba(124,58,237,0.12); }
.form-control-custom::placeholder { color: #c4b5fd; }
.field-hint { font-size: 0.76rem; color: #c4b5fd; margin-top: 5px; }

/* TOAST */
.toast-box {
    position: fixed; top: 24px; right: 24px; z-index: 9999; background: #fff;
    border-radius: 12px; box-shadow: 0 8px 30px rgba(109,40,217,0.15);
...
.toast-success .t-icon { background: #ede9fe; color: #8b5cf6; }
.toast-error .t-icon { background: #fef0f0; color: #dc2626; }
@keyframes slideIn { from { opacity: 0; transform: translateX(20px); } to { opacity: 1; transform: translateX(0); } }

/* VIEW CARD */
.view-card { background: #fff; border-radius: 16px; box-shadow: 0 2px 16px rgba(109,40,217,0.08); margin-bottom: 20px; overflow: hidden; }
.view-card-header { padding: 16px 24px; background: #f5f3ff; border-bottom: 1px solid #ddd6fe; display: flex; align-items: center; gap: 10px; }
.view-card-header h6 { font-size: 0.95rem; font-weight: 700; color: #6d28d9; margin: 0; }
.view-card-body { padding: 24px; }
.info-row { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 12px; }
.info-item label { font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; color: #c4b5fd; display: block; margin-bottom: 3px; }
...
.tl-dot.done    { background: #a78bfa; }
.tl-dot.pending { background: #ede9fe; border: 2px solid #c4b5fd; }
.tl-dot.active  { background: #f59e0b; }
.tl-content .tl-title { font-size: 0.875rem; font-weight: 600; color: #6d28d9; }
.tl-content .tl-date  { font-size: 0.78rem; color: #c4b5fd; }
.tl-content .tl-note  { font-size: 0.82rem; color: #6b7280; margin-top: 3px; }

/* EMPTY STATE */
.empty-state { text-align: center; padding: 60px 20px; color: #c4b5fd; }
.empty-state i { font-size: 3rem; margin-bottom: 12px; display: block; color: #ddd6fe; }

/* SISTEM TABLE */
.sistem-table { width: 100%; border-collapse: collapse; font-size: 0.85rem; }
.sistem-table th, .sistem-table td { padding: 10px 14px; border: 1px solid #ddd6fe; }
.sistem-table thead th { background: #f5f3ff; color: #6d28d9; font-weight: 700; font-size: 0.72rem; text-transform: uppercase; }
.sistem-table tbody tr:hover td { background: #f5f3ff; }
.sistem-table .check-col { width: 50px; text-align: center; }
.sistem-table input[type=checkbox] { width: 16px; height: 16px; cursor: pointer; accent-color: #8b5cf6; }
.sistem-table input[type=text] { border: 1px solid #ddd6fe; border-radius: 6px; padding: 5px 10px; font-size: 0.82rem; width: 100%; outline: none; background: #fff; }
.sistem-table input[type=text]:focus { border-color: #8b5cf6; box-shadow: 0 0 0 2px rgba(124,58,237,0.1); }

/* ACTION ROW */
.action-row { display: flex; align-items: center; gap: 10px; padding: 20px 24px; border-top: 1px solid #ddd6fe; background: #f5f3ff; }
