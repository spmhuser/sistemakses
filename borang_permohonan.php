                        ...
                        <tr id="row-<?=$bil?>">
                            ...
                            <td>
                                <select name="peranan_sistem_<?=$bil?>" id="ps-<?=$bil?>"
                                        ...
                                        style="border:1px solid #fef9c3;border-radius:6px;padding:5px 8px;font-size:0.8rem;width:100%;outline:none;background:#f9f9f9;color:#9ca3af;cursor:not-allowed">
                                ...
                                <?php foreach(SENARAI_FUNGSI as $f): ?>
                                    <label id="hklbl-<?=$bil?>-<?=$f?>"
                                        ...
                                        <input type="checkbox" name="had_kuasa_<?=$bil?>[<?=$f?>]" value="1"
                                               id="hk-<?=$bil?>-<?=$f?>" disabled style="accent-color:#facc15">
                                        <?= fungsiLabel($f) ?>
                                    </label>
                                <?php endforeach; ?>
                                </div>
...
function enableHadKuasa(bil) {
    senaraiFungsi.forEach(f => {
        ...
        if (!cb) return;
        cb.disabled = false;
        lbl.style.cssText = 'display:inline-flex;align-items:center;gap:3px;font-size:0.72rem;padding:2px 6px;border-radius:12px;border:1px solid #fde047;background:#fff;color:#374151;cursor:default;white-space:nowrap';
    });
    updateHadKuasa(bil);
}
...
function toggleRow(chk, bil) {
    ...
    if (chk.checked) {
        sel.disabled = false;
        sel.required = true;
        sel.style.cssText = 'border:1px solid #fde047;border-radius:6px;padding:5px 8px;font-size:0.8rem;width:100%;outline:none;background:#fff;color:#374151;cursor:default';
        cat.disabled = false;
        ...
        sel.required = false;
        sel.value = '';
        sel.style.cssText = 'border:1px solid #fef9c3;border-radius:6px;padding:5px 8px;font-size:0.8rem;width:100%;outline:none;background:#f9f9f9;color:#9ca3af;cursor:not-allowed';
        cat.value = '';
        cat.disabled = true;
