...
<head>
    ...
    <title>Borang Permohonan – Sistem Capaian Sistem</title>
    ...
    <style>
        ...
        .radio-card:hover{border-color:#1976d2;background:#eff6ff;}
        .radio-card.selected{border-color:#1976d2;background:#e8f0fe;}
        .radio-card input{accent-color:#1976d2;}
    </style>
</head>
<body>
...
<div class="main-content">
                        ...
                        <tr id="row-<?=$bil?>">
                            ...
                            <td>
                                <select name="peranan_sistem_<?=$bil?>" id="ps-<?=$bil?>"
                                        ...
                                        style="border:1px solid #e8f0fe;border-radius:6px;padding:5px 8px;font-size:0.8rem;width:100%;outline:none;background:#f9f9f9;color:#9ca3af;cursor:not-allowed">
                                ...
                                <?php foreach(SENARAI_FUNGSI as $f): ?>
                                    <label id="hklbl-<?=$bil?>-<?=$f?>"
                                        ...
                                        <input type="checkbox" name="had_kuasa_<?=$bil?>[<?=$f?>]" value="1"
                                               id="hk-<?=$bil?>-<?=$f?>" disabled style="accent-color:#1976d2">
                                        <?= fungsiLabel($f) ?>
                                    </label>
                                <?php endforeach; ?>
                                </div>
    ...
    <div class="form-card mb-4">
        ...
        <div class="form-section-body">
            <div style="background:#eff6ff;border:1px solid #e8f0fe;border-radius:10px;padding:16px;margin-bottom:20px;font-size:0.875rem;color:#374151">
...
function enableHadKuasa(bil) {
    senaraiFungsi.forEach(f => {
        ...
        if (!cb) return;
        cb.disabled = false;
        lbl.style.cssText = 'display:inline-flex;align-items:center;gap:3px;font-size:0.72rem;padding:2px 6px;border-radius:12px;border:1px solid #bfdbfe;background:#fff;color:#374151;cursor:default;white-space:nowrap';
    });
    updateHadKuasa(bil);
}
...
function toggleRow(chk, bil) {
    ...
    if (chk.checked) {
        sel.disabled = false;
        sel.required = true;
        sel.style.cssText = 'border:1px solid #bfdbfe;border-radius:6px;padding:5px 8px;font-size:0.8rem;width:100%;outline:none;background:#fff;color:#374151;cursor:default';
        cat.disabled = false;
        ...
        sel.required = false;
        sel.value = '';
        sel.style.cssText = 'border:1px solid #e8f0fe;border-radius:6px;padding:5px 8px;font-size:0.8rem;width:100%;outline:none;background:#f9f9f9;color:#9ca3af;cursor:not-allowed';
        cat.value = '';
        cat.disabled = true;
